...
cerr << "Spaces: " << spaces << "\n";
  ...
  edges[i] = make_pair(atol(W.Strings[spaces*i]), atol(W.Strings[spaces*i + 1]));
    ...
    std::sort(edges.begin(), edges.begin()+m, cmpBoth());
    using T=pair<int,int>;
    T lastRead = make_pair(-1, -1);
    long offset = 0;
    for (long i=0; i < m; ++i) {
...
if (nu > LONG_MAX - nv) {cerr << "err\n"; exit(0);}
    ...
    long* degV = newA(long, nv);
    long* degU = newA(long, nu);
    for(long i=0; i < nv; ++i) {degV[i] = 0;}
    for(long i=0; i < nu; ++i) {degU[i] = 0;}

    long* idxV = newA(long, nv);
    long* idxU = newA(long, nu);
    for(long i=0; i < nv; ++i) {idxV[i] = 0;}
    for(long i=0; i < nu; ++i) {idxU[i] = 0;}
    ...
    for (long i=0; i < nu; ++i) {
      (adj[nv+i]).resize(degU[i]);
//if (nv + i == 1177538402) { cerr << degU[i] << "\n"; adj[nv+i][0] = 0; }
    }
    ...
    //adj[B].push_back(A);
//if(A >= nv) {cerr << "err1\n"; exit(0);}
//if(idxV[A] >= degV[A]) {cerr << idxV[A] << ", " << degV[A] << "eerr2\n"; exit(0);}
adj[A][idxV[A]] = B;
adj[B][idxU[edge.second]] = A;
idxV[A]++;
idxU[edge.second]++;
  }
...
free(degU);
  ...
  for (long i = largest_index_in_partition[0]; i < largest_index_in_partition[1]; i++) {
...
cerr << "reading file time: " << elapsed1 << "\n";
...
cerr << "creating graph: " << elapsed2 << "\n";
clock_t begin3 = clock();
	resize_all();
clock_t end3 = clock();
double elapsed3 = (double)(end3-begin3) / CLOCKS_PER_SEC;
cerr << "resizing arrays: " << elapsed3 << "\n";
clock_t begin = clock();
	n_wedge_in_partition[0] = 0;
...
cerr << "wedge computations: " << elapsed << "\n";
...
ll exact_butterfly_counting(vector < vector <int> > &graph) {
	...
	ld res = 0;
	for (int vertex = side == 0 ? 0 : nv ; vertex < largest_index_in_partition[side]; vertex++) {
		int idx = 0;
...
butterflies[vertex] += hashmap_C[two_hop_neighborhood];
butterflies[two_hop_neighborhood] += hashmap_C[two_hop_neighborhood];
